...
import algorithm.hierarchy_tree as h_tree  # custom library

def summarized(partition, qi_list):
    for qi in qi_list:
        partition.sort_values(by=qi, inplace=True)
        first, last = partition[qi].iloc[0], partition[qi].iloc[-1]
        if first != last:
            partition[qi] = f"{first}-{last}"
    return partition
...
def anonymize(partition, ranks, k, qi_list):
    ...
    mid = len(partition) // 2
    left_partition, right_partition = partition.iloc[:mid], partition.iloc[mid:]
    if len(left_partition) >= k and len(right_partition) >= k:
        return pd.concat([anonymize(left_partition, ranks, k, qi_list),
                          anonymize(right_partition, ranks, k, qi_list)])
    return summarized(partition, qi_list)

def mondrian(partition, qi_list, k):
    ranks = sorted(((qi, partition[qi].nunique()) for qi in qi_list),
...
def map_num_to_text(df, qi_list, hierarchy_tree_dict):
    for column in qi_list:
        hierarchy_tree = hierarchy_tree_dict[column]
        df[column] = df[column].apply(lambda x: hierarchy_tree.leaf_id_dict[str(x)].value if str(x).isdigit()
        else hierarchy_tree.find_common_ancestor(*map(str, x.split('-'))).value)
    return df

def check_k_anonymity(df, qi_list, k):
    return df.groupby(qi_list).size().min() >= k
...
def run_anonymize(qi_list, data_file, hierarchy_file_dir, k=5):
    ...
    if df.isnull().values.any():
        ...
        print("Warning: NaN values found and filled with 1. Please handle them before anonymizing.")
    df = mondrian(df, qi_list, k)
    if not check_k_anonymity(df, qi_list, k):
        raise ValueError("Not all partitions are k-anonymous")
    return map_num_to_text(df, qi_list, hierarchy_tree_dict)

def anonymize_execute(k_value):
    tic = time.time()
    print(f"Running anonymization with K = {k_value}")

    current_dir = os.path.dirname(__file__)
    parent_dir = os.path.dirname(current_dir)

    input_path = os.path.join(parent_dir, "input", "dataset.csv")
    hierarchy_file_path = os.path.join(current_dir, "hierarchy")

    quasi_identifiers = ['sex', 'age', 'race', 'marital-status', 'education', 'native-country', 'workclass', 'occupation']

    data_frame = run_anonymize(quasi_identifiers, input_path, hierarchy_file_path, k_value)

    output_file_path = os.path.join(parent_dir, "output", "anonymized", f'k_{k_value}_anonymized_dataset.csv')
    try:
